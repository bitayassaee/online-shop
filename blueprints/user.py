...
def login():
    if request.method == "GET":
        ...
        if register != None:
            ...
            user = User.query.filter(User.username == username).first()
            if user == None:
                flash('نام کاربری یا رمز اشتبه است')
                return redirect(url_for('user.login'))
...
def payment():
    ...
    r = requests.post(confing.PAYMENT_FIRST_REQUEST_URL,
                      data={
                          'api': confing.PAYMENT_MERCHANT,
                          'amount': cart.total_price(),
                          'callback': confing.PAYMENT_CALLBACK
                            })
...
def varify():
    ...
    r = requests.post(confing.PAYMENT_VERIFY_REQUEST_URL,
                      data={
                          'api': 'sandbox',
                          'amount': pay.price,
                          'token': token
